...
## read Hillstrom data
data <- fread('data/hillstrom.csv')

## delete menth closing campaign and encode treatment
data <- data[!(segment == "Mens E-Mail"),]
data[, W := ifelse(segment == 'Womens E-Mail', 1, 0)]
data$segment <- NULL
y_data <- data[, target, with = F]
data$Y <- data[, target, with = F]
data <- data[,-c('conversion', 'spend', 'visit'), with = F]
data$history_segment <- NULL
data <- as.data.table(model.matrix(Y~-1+., data))
data$Y <- y_data
data$history <- NULL

## split in training and test set
idx_train <- sample(1:nrow(data), round(0.8*nrow(data)), replace = F)
